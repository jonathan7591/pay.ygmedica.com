...
class chinaums_plugin
{
	...
	static private function handleProfits(&$param){
		...
		if($psreceiver){
			$psmoney = intval(round(floor($order['realmoney'] * $psreceiver['rate'])));
			$param['divisionFlag'] = true;
			$param['platformAmount'] = $param['totalAmount'] - $psmoney;
			$param['subOrders'] = [
				[
					'mid' => $psreceiver['account'],
					'merOrderId' => $channel['msgsrcid'].TRADE_NO.'1',
					'totalAmount' => $psmoney
				]
			];
		}
