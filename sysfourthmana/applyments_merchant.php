...
$channels = $DB->getAll("SELECT * FROM pre_applychannel order by sort asc");
...
<input type="hidden" class="form-control" name="batch">
  <div class="form-group">
    <label>搜索</label>
	<select name="column" class="form-control"><option value="orderid">申请单号</option><option value="thirdid">第三方单号</option><option value="mchname">商户名称</option><option value="mchid">子商户号</option></select>
  </div>
  <div class="form-group">
    <input type="text" class="form-control" name="value" placeholder="搜索内容">
  </div>
  <div class="form-group">
    <input type="text" class="form-control" name="uid" style="width: 100px;" placeholder="用户ID" value="">
  </div>
  <div class="form-group">
			...
			},
			{
				...
				formatter: function(value, row, index) {
					...
					if(row.subchannel){
						...
						if(row.subchannel_count == 1){
							html += '<li><a onclick="testpay('+row.subchannel+','+row.channel+')">支付测试</a></li>';
						}
					}
					html += '<li><a href="javascript:changeUid('+row.id+','+row.uid+')">修改绑定本站UID</a></li>';
...
function removeMerchant(id){
	var confirmobj = layer.confirm('确定要对此商户进行作废处理吗？作废后将不再能够进行收单结算', {
	  btn: ['确定','取消'], icon:0
	}, function(){
	  $.ajax({
		type : 'POST',
		url : 'ajax_applyments.php?act=removeMerchant',
		data : {id:id},
		dataType : 'json',
		success : function(data) {
			if(data.code == 0){
...
function changeUid(id, uid){
	layer.prompt({title: '修改绑定用户UID', value: uid, formType: 0}, function(text, index){
		var ii = layer.load(2, {shade:[0.1,'#fff']});
		$.ajax({
			type : 'POST',
			url : 'ajax_applyments.php?act=changeUid',
			data : {id:id,uid:text},
			dataType : 'json',
			success : function(data) {
				layer.close(ii);
...
function getAll(type, channel, obj){
	...
	$.ajax({
		type : 'GET',
		url : 'ajax_pay.php?act=getSubChannelMoney&type='+type+'&channel='+channel,
		dataType : 'json',
		success : function(data) {
			layer.close(ii);
			if(data.code == 0){
...
function showChannelStatus(uid, id){
	...
	$.ajax({
		...
		success : function(data) {
			layer.close(ii);
			var content = '<table class="table table-striped"><thead><tr><th>ID</th><th>支付方式</th><th>支付通道</th><th>状态</th><th>操作</th></tr></thead><tbody>';
			$.each(data, function(i, item){
				content += '<tr><td>'+item.id+'</td><td><img src="/assets/icon/'+item.type+'.ico" width="16" onerror="this.style.display=\'none\'"> '+item.typename+'</td><td>'+item.channel+'_'+item.channelname+'</td><td>'+(item.status==1?'<a class="btn btn-xs btn-success" onclick="setChannelStatus(this,'+item.id+',0)">已开启</a>':'<a class="btn btn-xs btn-danger" onclick="setChannelStatus(this,'+item.id+',1)">已关闭</a>')+'</td><td><a href="./order.php?subchannel='+item.id+'" target="_blank" class="btn btn-xs btn-default">订单</a>&nbsp;<a onclick="testpay('+item.id+','+item.channel+')" class="btn btn-xs btn-default">测试</a></td></tr>';
			});
			content += '</tbody></table>';
			layer.open({
				type: 1,
				...
				title: '单独支付方式开关',
				content: content,
				area: [$(window).width() > 480 ? '480px' : '100%', ';max-height:100%']
			});
		},
		error:function(data){
			layer.msg('服务器错误');
