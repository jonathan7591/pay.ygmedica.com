        ...
        (function ($) {
            ...
            ZdCascader.METHODS = ['reload', 'destroy', 'initFormData'];
            ...
            ZdCascader.prototype.initFormData = function (data) {
                this.options.initData = data;
                this._initFromData();
            }

            ZdCascader.prototype._initFromData = function () {
              if (typeof this.options.initData == "object" && this.options.initData&&this.options.initData.length>0) {
                const initData = this.options.initData.concat();
                initData.reverse();
                var label = [];
                this.options.data.forEach(function (prov) {
                    if (prov.value == initData[0]) {
                        label.push(prov.label);
                        if (initData.length > 1 && prov.children && prov.children.length > 0) {
                            prov.children.forEach(function (city) {
                                if (city.value == initData[1]) {
                                    label.push(city.label);
                                    if (initData.length > 2 && city.children && city.children.length > 0) {
                                        city.children.forEach(function (area) {
                                            if (area.value == initData[2]) {
                ...
                var label = label.join(this.options.range);
                this.$el.val(label);
                this.$val_el.val(initData);
                this.$el.data('persist_v', label);
              } else if (typeof this.options.initData == "string" && this.options.initData) {
                ...
                try{
                    this.options.data.forEach(function (prov) {
                        label.push(prov.label);
                        value.push(prov.value);
                        if (prov.value == initData) {
                            throw Error()
                        }
                        if (prov.children && prov.children.length > 0) {
                            prov.children.forEach(function (city) {
                                ...
                                if (city.value == initData) {
                                    throw Error()
                                }
                                if (city.children && city.children.length > 0) {
                                    city.children.forEach(function (area) {
                                        label.push(area.label);
                                        value.push(area.value);
                                        if (area.value == initData) {
                                            throw Error()
                                        }else{
                                            label.pop();
                                            value.pop();
            ...
            ZdCascader.prototype._loadChildren = function ($parentNode) {
                ...
                $.each(data.children, (i, m) => {
                    ...
                    if(m.desc){
                        var $label = $(`<span class="${this.CLASS.menuNodeLabel}" title="${m.desc}">${m.label}</span>`);
                    }else{
