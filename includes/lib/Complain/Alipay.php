...
class Alipay implements IComplain
{
    ...
    private function updateInfo($info){
        ...
        if($row){
            ...
            if($order || $conf['complain_range']==1){
                $DB->insert('complain', ['paytype'=>$this->channel['type'], 'channel'=>$this->channel['id'], 'subchannel'=>$order['subchannel'] ?? 0, 'uid'=>$order['uid'] ?? 0, 'trade_no'=>$trade_no, 'thirdid'=>$thirdid, 'type'=>$info['leaf_category_name'], 'title'=>$info['complain_reason'], 'content'=>$info['content'], 'status'=>$status, 'phone'=>$info['phone_no'], 'addtime'=>$info['gmt_create'], 'edittime'=>$info['gmt_modified']]);
    ...
    public function feedbackSubmit($thirdid, $code, $content, $images = []){
        if(empty($code) || $code == 'ORTHER') $code = '03';
